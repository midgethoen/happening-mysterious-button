...
exports.client_press = (data)->
  name = Plugin.userName(data.userId)

  pos = Db.shared.incr 'num_things'
  Db.shared.set 'things', pos, sentence(name, data.thing)

  Event = require 'event'
  Event.create
    text:  sentence(name, data.thing)

sentence = (name, thing) ->
  name + ' vindt Midge ' + thing
